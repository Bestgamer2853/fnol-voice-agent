...
export class GroqService implements LlmProvider {
  ...
  constructor(options: GroqServiceOptions = {}) {
    this.apiKey = options.apiKey ?? readEnvironmentValue('GROQ_API_KEY');
  ...
  async generateResponse(
    ...
    if (!this.apiKey) {
      return fallbackResponse('Groq API Key is not configured.');
    }
    ...
    while (attempt <= MAX_RETRIES) {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          console.error(`[LLM Response Error] Groq Attempt ${attempt}, Status: ${response.status}, Latency: ${latency}ms, Error: ${errorBody}`);

          if (attempt <= MAX_RETRIES && RETRYABLE_STATUS_CODES.has(response.status)) {
            const backoff = 200 + Math.random() * 100;
            console.log(`[LLM Retry] Groq Status ${response.status}. Retrying in ${Math.round(backoff)}ms...`);
            await sleep(backoff);
            continue;
          }
            ...
            if (chunk.startsWith('data: ')) {
              ...
              if (dataStr !== '[DONE]') {
                try {
                  ...
                  console.error('Failed to parse Groq SSE chunk', e);
                }
              }
            }
        ...
        console.log(`[LLM Response Success] Groq Latency: ${latency}ms`);
        ...
        if (attempt <= MAX_RETRIES) {
          const backoff = 200 + Math.random() * 100;
          console.log(`[LLM Retry] Groq Network Error. Retrying in ${Math.round(backoff)}ms...`);
          await sleep(backoff);
          continue;
        }
...
export function createGroqService(options: GroqServiceOptions = {}): LlmProvider {
  return new GroqService(options);
}
