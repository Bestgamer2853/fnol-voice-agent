...
export class GroqService implements LlmProvider {
  ...
  constructor(options: GroqServiceOptions = {}) {
    ...
    this.apiKey = options.apiKey ?? groqKey;
  ...
  async generateResponse(
    ...
    while (attempt <= MAX_RETRIES) {
      ...
      console.log(`[Groq LLM Request] Attempt ${attempt}. URL: ${url}, Model: ${this.model}, Tools: ${openaiTools?.length || 0}`);
      ...
      try {
        ...
        if (!response.ok) {
          ...
          console.error(`[Groq Response Error] Attempt ${attempt}, Status: ${response.status}, Latency: ${latency}ms, Error: ${errorBody}`);
          ...
          return fallbackResponse(`Groq request failed with status ${response.status}: ${errorBody}`);
        }
